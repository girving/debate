...
def Alice' (ι : Type) (o : OracleId) := ι → BComp ι {o} ℝ
...
/-- Bob sees the oracle input and Alice's claimed probability, and decides whether to continue.
    ...
    mimic the oracle. -/
def Bob' (ι : Type) (o : OracleId) := ι → ℝ → BComp ι {o} Bool
...
def Vera (ι : Type) := ι → ℝ → BComp ι {VeraId} Bool
...
def step (alice : Alice ι) (bob : Bob ι) (vera : Vera ι) (y : ι) : BComp ι AllIds (State Bool) := do
...
def debate (alice : Alice ι) (bob : Bob ι) (vera : Vera ι) (f : BComp ι s Bool) :
    BComp ι AllIds Bool := do
...
structure Correct (f : BComp ι s Bool) (w k : ℝ) (alice : Alice ι) (bob : Bob ι) (vera : Vera ι) :
    Prop where
  /-- Success is more likely than failure -/
  half_lt_w : 1/2 < w
  /-- If we're in the language, Alice wins -/
  complete : ∀ (o : BOracle ι) (eve : Bob ι), f.lipschitz o k → 2/3 ≤ (f.prob' o).prob true →
    w ≤ ((debate alice eve vera f).prob' o).prob true
  /-- If we're out of the language, Bob wins -/
  sound : ∀ (o : BOracle ι) (eve : Alice ι), f.lipschitz o k → 2/3 ≤ (f.prob' o).prob false →
