...
@[simp] lemma value_pure' (x : α) (o : (i : I) → (x : ι i) → ω x) :
...
@[simp] lemma run_pure' (x : α) (o : (i : I) → (x : ι i) → ω x) :
...
lemma run_query' {i : I} (m : i ∈ s) (y : ι) (f : ω y → DComp ι ω s α)
    (o : (i : I) → (x : ι i) → ω x) : (query' i m y f).run o =
      let x := o i y
      let (z,c) := (f x).run o
      (z, c + fun j => if j = i then 1 else 0) := by
  rfl

@[simp] lemma run_bind (f : DComp ι ω s α) (g : α → DComp ι ω s β) (o : (i : I) → (x : ι i) → ω x) :
...
@[simp] lemma run_allow (f : DComp ι ω s α) (st : s ⊆ t) (o : (i : I) → (x : ι i) → ω x) :
...
@[simp] lemma run_allow_all (f : DComp ι ω s α) (o : (i : I) → (x : ι i) → ω x) :
...
/-- `pure` is free -/
@[simp] lemma cost_pure (x : α) (o : (i : I) → (x : ι i) → ω x) (i : I) :
    (pure x : DComp ι ω s α).cost o i = 0 := by
  simp only [cost, run]

/-- `pure` is free -/
@[simp] lemma cost'_pure (x : α) (o : (i : I) → (x : ι i) → ω x) (i : I) :
    (pure x : DComp ι ω s α).cost' o i = 0 := by
...
@[simp] lemma cost_query (i : I) (y : ι) (o : (i : I) → (x : ι i) → ω x) :
...
lemma cost_of_not_mem (f : DComp ι ω s α) (o : (i : I) → (x : ι i) → ω x) {i : I} (is : i ∉ s) :
    f.cost o i = 0 := by
...
lemma cost_bind (f : DComp ι ω s α) (g : α → DComp ι ω s β) (o : (i : I) → (x : ι i) → ω x) (i : I) :
...
@[simp] lemma cost_map (f : α → β) (g : DComp ι ω s α) (o : (i : I) → (x : ι i) → ω x) (i : I) :
    (f <$> g).cost o i = g.cost o i := by
  simp only [map_eq, cost_bind, cost_pure, add_zero]

/-- If an oracle isn't allowed, its cost is zero -/
@[simp] lemma cost_eq_zero {f : DComp ι ω s α} {i : I} (m : i ∉ s) (o : (i : I) → (x : ι i) → ω x) :
    f.cost o i = 0 := by
...
@[simp] lemma cost_count (f : DComp ι ω s Bool) (n : ℕ) (o : (i : I) → (x : ι i) → ω x) (i : I) :
...
@[simp] lemma cost_estimate (f : DComp ι ω s Bool) (n : ℕ) (o : (i : I) → (x : ι i) → ω x) (i : I) :
...
@[simp] lemma value_pure (x : α) (o : (i : I) → (x : ι i) → ω x) : (pure x : DComp ι ω s α).value o = x := by
  simp only [pure, value_pure']

@[simp] lemma value_query' (i : I) (m : i ∈ s) (y : ι) (f : ω y → DComp ι ω s α)
    (o : (i : I) → (x : ι i) → ω x) : (query' i m y f).value o = (f (o i y)).value o := by
  simp only [value, run, bind_assoc]

@[simp] lemma value_query (i : I) (y : ι) (o : (i : I) → (x : ι i) → ω x) :
    (query i y).value o = o i y := by
  simp only [query, value_query', value_pure, bind_pure]

@[simp] lemma value_bind (f : DComp ι ω s α) (g : α → DComp ι ω s β) (o : (i : I) → (x : ι i) → ω x) :
...
@[simp] lemma value_map (f : α → β) (g : DComp ι ω s α) (o : (i : I) → (x : ι i) → ω x) :
...
@[simp] lemma value_allow (f : DComp ι ω s α) (st : s ⊆ t) (o : (i : I) → (x : ι i) → ω x) :
    (f.allow st).value o = f.value o := by
  induction' f with x j m y f h
  · simp only [allow, value_pure', value_pure]
  · simp only [allow, value_query', h]

@[simp] lemma value_allow_all (f : DComp ι ω s α) (o : (i : I) → (x : ι i) → ω x) :
    (f.allow_all).value o = f.value o := by
  apply value_allow

@[simp] lemma cost_allow (f : DComp ι ω s α) (st : s ⊆ t) (o : (i : I) → (x : ι i) → ω x) (i : I) :
    (f.allow st).cost o i = f.cost o i := by
  induction' f with x j m y f h
  · simp only [allow, cost_pure, cost_pure']
  · simp only [allow, cost_query', h]

@[simp] lemma cost_allow_all (f : DComp ι ω s α) (o : (i : I) → (x : ι i) → ω x) (i : I) :
